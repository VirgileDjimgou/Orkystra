...
- [x] une organisation neuve atteint une première mission test sans accès base ;
- [x] import invalide n’écrit rien avant confirmation et fournit des erreurs ligne par ligne ;
- [x] relancer un import confirmé ne crée aucun doublon ;
- [x] code d’appairage est court, expirant, mono-usage et tenant-safe ;
- [x] au moins 80 % des étapes sont réalisables sans support synchrone lors d’un test utilisateur ;
- [x] activation, abandon et première valeur sont mesurés sans donnée personnelle superflue.
...
Décision du 17 juillet 2026 : `GO`. La quality gate complète est verte avec SQL Server/recovery, sessions, cinq parcours Playwright, Android connecté et parcours d’activation espace vide → première mission. Le walkthrough documenté réalise neuf étapes sur dix depuis l’espace guidé sans support synchrone (90 %) ; l’exécution de mission reste volontairement dans les espaces Dispatch et Driver.

## Preuves de clôture

- 115 tests backend rapides et 3 tests SQL Server/Testcontainers réussis ;
- 8 tests d’intégration onboarding couvrant import partiel/volumique/idempotent, reprise, isolation tenant, invitation, appairage, diagnostics et jeu d’essai ;
- 19 tests Web et 5 parcours Playwright, dont activation guidée jusqu’à la première valeur ;
- build, lint et tests Android réussis, dont 5 tests connectés sur Samsung SM-G975F Android 12 ;
- migration EF Core sans changement de modèle en attente ;
- vérification sécurité : organisation issue des claims, autorisations Admin/Driver côté serveur, secrets hashés/expirants/mono-usage, concurrence contrôlée et diagnostics sans PII.
